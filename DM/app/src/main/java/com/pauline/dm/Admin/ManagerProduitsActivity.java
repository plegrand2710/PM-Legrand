...
public class ManagerProduitsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        synchroniserEtChargerProduits();

        btnAddProduct.setOnClickListener(v -> openAddProductDialog());
        btnBack.setOnClickListener(v -> finish());
    }

    private void synchroniserEtChargerProduits() {
        synchroniserProduitsDistants(() -> {
            loadProducts();
        });
    }

    private void synchroniserProduitsDistants(Runnable callback) {
        new GestionnaireSelectBDDDistante(this, "produit", dbAdapter);

        productListView.postDelayed(() -> {
            Toast.makeText(this, "Synchronisation des produits réussie.", Toast.LENGTH_SHORT).show();
            callback.run();
        }, 2000);
    }
