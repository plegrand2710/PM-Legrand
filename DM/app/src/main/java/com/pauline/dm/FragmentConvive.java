...
public class FragmentConvive extends Fragments {
    ...
    // essaye de sauvegarder les commandes de chaque convives
    /*public ConviveCommande getCommande() {
        ConviveCommande commande = new ConviveCommande();
        AutoCompleteTextView temp = view.findViewById(R.id.platEditText);
        commande.set_plat(temp.getText().toString());
        commande.set_accompagnement(autoCompleteAccompagnement.getText().toString());
        commande.set_boisson(autoCompleteBoisson.getText().toString());
        return commande;
    }*/
}
